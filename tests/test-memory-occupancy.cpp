...
static bool check_primary_occupancy(llama_context * ctx, uint64_t & used_entries) {
    const llama_memory_primary_occupancy occupancy = llama_get_memory_primary_occupancy(ctx);
    const llama_memory_diagnostics diagnostics = llama_get_memory_diagnostics(ctx);

    if (!occupancy.available || diagnostics.state != "available") {
...
int main(int argc, char ** argv) {
    ...
    if (!llama_memory_seq_rm(llama_get_memory(ctx.get()), 0, -1, -1)) {
        fprintf(stderr, "%s: failed to remove sequence 0\n", __func__);
        return 1;
    }
