...
| `--kv-swap, --no-kv-swap` | swap unified attention KV pages to pinned host memory (default: disabled, requires --kv-unified)<br/>(env: LLAMA_ARG_KV_SWAP) |
| `--kv-swap-max-mib N` | maximum pinned host memory for KV swapping in MiB (default: 4096)<br/>(env: LLAMA_ARG_KV_SWAP_MAX_MIB) |
