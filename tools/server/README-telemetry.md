...
The server limits every target `llama_decode()` dispatch, including a recovered batch-size restore, to `min(n_batch, n_ubatch)`. One server dispatch therefore has exactly one physical target ubatch. Immediately before that dispatch, the server snapshots the latest desired control state and applies only the three microbatch controls. A successful application emits a trace-free schema-v1 `telemetry_control_boundary` event before any routing chunk. The event has the ring `sequence` and `server_instance_id`, `timestamp_unix_ms`, `props_generation`, a separate `microbatch_generation`, `physical_step` (the cumulative successful target-ubatch ordinal), `physical_microbatch` (always zero within this one-ubatch dispatch), the full `effective` seven-toggle state, `native_moe_routing_enabled`, and `moe_routing_applicable`. A newer desired microbatch state that arrives before a dispatch overwrites the pending one; it does not create a fabricated boundary for the older generation.

The microbatch generation changes only when `moe_routing`, `kv_pressure_detail`, or `native_gpu_gpm` changes. A routed context calls `llama_set_moe_routing()` only when its actual native capture state transitions. Enabling or disabling after a captured dispatch can synchronize the previous capture once; steady disabled paths and dense models do not call it, allocate routing output, read it back, synchronize it, or inspect batch peers.
