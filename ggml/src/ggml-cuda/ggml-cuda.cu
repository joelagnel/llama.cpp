...
// Native Windows on some unified-memory platforms exposes dedicated + shared
// memory through cudaMemGetInfo, but cudaMalloc allocations that cross the
// dedicated carveout can still fail only when first touched.  CUDA mapped host
// memory is GPU-addressable on those devices and provides a deterministic
// fallback.  Keep this opt-in because zero-copy memory is substantially slower
// than local VRAM on conventional discrete GPUs.
